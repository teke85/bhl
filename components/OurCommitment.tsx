...
export default function OurCommitment({
  ...
  description = "We are committed to delivering the Western Corridor Project as a transformative infrastructure initiative that strengthens Zambia's regional connectivity, economic resilience, and social equity. Through Barotse our dedicated Special Purpose Vehicle,",
  points = [
  ...
  footerDescription = "Our commitment is to not only construct a road, but to create a corridor of opportunity, sustainability, and shared prosperity for Zambia and the region.",
}: OurCommitmentProps) {
  return (
    <section className="w-full py-16 md:py-14 bg-background dark:bg-[#0a0a0a]">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 gap-12 justify-between items-center">
          <div>
            ...
            <p className="text-lg text-muted-foreground dark:text-white font-normal font-body mb-8">
              {description}
            </p>

            <div className="space-y-5 font-body mb-4">
              {points.map((item, index) => (
                <div key={index} className="flex items-start gap-4">
                  <div className="w-2 h-2 rounded-full bg-[#fdb913] mt-2 shrink-0"></div>
                  <span className="text-base text-muted-foreground dark:text-white leading-relaxed">
                    {item}
                  </span>
                </div>
              ))}
